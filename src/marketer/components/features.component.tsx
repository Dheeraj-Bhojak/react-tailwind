...
//featuresArry contains all features object
const featuresArray: {
  id: number;
  title: string;
  description: string;
...
const Features: React.FC = () => {
  return (
    <>
      ...
      <section className="text-gray-800 text-center mx-auto px-4 sm:px-6 lg:px-4 py-12 bg-gray-100">
        <div className="flex justify-center">
          <div className="text-center max-w-[700px]">
            <h2 className="text-3xl font-bold mb-6 text-center">
              Why is it so<u className="text-indigo-600"> great?</u>
            </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 lg:gap-x-12">
          {featuresArray.map((feature) => (
            <div key={feature.id} className="mb-12 lg:mb-0">
              <div
                key={feature.id}
                className="p-4 bg-indigo-600 rounded-full shadow-lg inline-block mb-6"
              >
                <svg
              ...
              <h5 className="text-lg font-bold mb-4">{feature.title}</h5>
